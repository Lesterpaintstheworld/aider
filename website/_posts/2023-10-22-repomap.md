...
excerpt: Tree-sitter allows aider_nova to build a repo map that better summarizes large code bases.
...
To address these issues, aider_nova
...
aider_nova now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [aider_nova](https://aider_nova.chat/docs/install.html).*
...
aider_nova also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like aider_nova to automatically
identify and provide the needed code context.

## Using a repo map to provide context

aider_nova sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and aider_nova will automatically add them to the chat context.
...
aider_nova uses all of these definitions and references to
...
[ctags based map](https://aider_nova.chat/docs/ctags.html)
that aider_nova originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install aider_nova-chat`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for aider_nova.
...
Right now, aider_nova relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using aider_nova's `/add` command,
...
just install [aider_nova](https://aider_nova.chat/docs/install.html).

## Credits

aider_nova uses
[modified versions of the tags.scm files](https://github.com/paul-gauthier/aider_nova/tree/main/aider_nova/queries)
from these
